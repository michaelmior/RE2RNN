...
def load_SMS_dataset():
    testDf = (
        pd.read_csv(os.path.join(SMS_PATH, "test.csv"))
        .reset_index(drop=True)
        .rename({"v1": "class", "v2": "text"}, axis="columns")
    )
    testDf["mode"] = "test"
    testDf["text"] = testDf["text"].str.lower()
    trainDf = (
        pd.read_csv(os.path.join(SMS_PATH, "train.csv"))
        .reset_index(drop=True)
        .rename({"v1": "class", "v2": "text"}, axis="columns")
    )
    trainDf["mode"] = "train"
    ...
    validDf = (
        pd.read_csv(os.path.join(SMS_PATH, "valid.csv"))
        .reset_index(drop=True)
        .rename({"v1": "class", "v2": "text"}, axis="columns")
    )
    validDf["mode"] = "valid"
    ...
    indexToWord, wordToIndex = get_word_to_index(
        map(lambda text: tokenizer(text) + text.split(" "), list(df["text"]))
    )
    return {
...
def load_TREC_dataset():
    ...
    df = pd.DataFrame(zip(classes, texts, modeTag)).rename(
        columns={0: "class", 1: "text", 2: "mode"}
    )
    textsToIndex = list(
        map(lambda text: tokenizer(text) + text.split(" "), list(df["text"]))
    )

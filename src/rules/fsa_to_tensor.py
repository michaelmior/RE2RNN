...
def dfa_to_tensor(
    automata, word2idx: Dict[str, int], subtype=False, disturb_func=subtype_disturb_func
):
    """
    Parameters
    ----------
    ...
    state2idx = {state: idx for idx, state in enumerate(all_states)}
    ...
    for fr_state, to in sorted(automata["transitions"].items()):
        for to_state, to_edges in sorted(to.items()):
            for edge in to_edges:
                val = 1
                if (subtype) and (
                    (fr_state in automata["subtypes"])
                    or (to_state in automata["subtypes"])
                ):  # add subtype distortion
                    val = disturb_func(1)

                if edge == "&":  # punctuations
                    tensor[
                        list(punct_idxs.values()),
                        state2idx[fr_state],
                        state2idx[to_state],
                    ] = val
                elif edge == "%":  # digits
                    tensor[
                        list(number_idxs.values()),
                        state2idx[fr_state],
                        state2idx[to_state],
                    ] = val
                elif edge == "$":
                    ...
                    if edge in word2idx:
                        tensor[
                            word2idx[edge], state2idx[fr_state], state2idx[to_state]
                        ] = val
                        language.add(edge)
                    else:
    ...
    def addtransition(self, fromstate, tostate, inp):
        ...
        if fromstate in self.transitions:
            if tostate in self.transitions[fromstate]:
                self.transitions[fromstate][tostate] = self.transitions[fromstate][
                    tostate
                ].union(inp)
            else:

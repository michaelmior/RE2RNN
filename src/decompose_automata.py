...
def decompose_automata(args):
    merged_automata = load_pkl(
        "../data/{}/automata/{}".format(args.dataset_name, args.automata_name)
    )
    ...
    for random_state in range(4):
        print(
            "DECOMPOSING RANK: {}, TENSOR SIZE: {}".format(
                args.rank, language_tensor.shape
            )
        )
        V_embed_split, D1_split, D2_split, rec_error = decompose_tensor_split(
            language_tensor,
            language,
            word2idx,
            args.rank,
        ...
        pickle.dump(
            save_dict,
            open(
                "../data/{}/automata/automata.{}.{:.4f}.{}.pkl".format(
                    args.dataset_name, args.rank, rec_error[-1], random_state
                ),
                "wb",
            ),
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--automata_name",
        type=str,
        default="all.1030104910-1604054950.712768.split.pkl",

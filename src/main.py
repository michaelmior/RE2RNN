...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--lr", type=float, default=0.001, help="learning rate of optimizer"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--model_id", type=str, default="none", help="model id, identifying model"
    )
    parser.add_argument(
        "--activation",
        type=str,
    ...
    parser.add_argument(
        "--train_wildcard", type=int, default=0, help="If we train the wildcard matrix"
    )
    parser.add_argument(
        "--train_linear", type=int, default=0, help="if we train the linear parameters"
    )
    parser.add_argument(
        "--train_V_embed",
        type=int,
    ...
    parser.add_argument(
        "--train_beta", type=int, default=0, help="if we train the vector beta"
    )
    parser.add_argument(
        "--clip_neg",
        type=int,
        default=0,
        help="if we use relu after each hidden update",
    )
    parser.add_argument(
        "--sigmoid_exponent",
        type=int,
        default=5,
        help="exponent in TENSORFSAGRU sigmoid",
    )
    parser.add_argument(
        "--beta",
        type=float,
    ...
    parser.add_argument(
        "--farnn", type=int, default=0, help="0 if use farnn, 1 if use fagru"
    )
    parser.add_argument("--bias_init", type=float, default=7, help="bias_init for gru")
    parser.add_argument(
        "--reg_type", type=str, default="nuc", help="nuclear norm or fro norm"
    )
    parser.add_argument(
        "--wfa_type", type=str, default="viterbi", help="forward or viterbi"
    )
    parser.add_argument(
        "--random_embed", type=int, default=0, help="random embedding or not"
    )
    parser.add_argument(
        "--clamp_score", type=int, default=0, help="if we clamp the score in [0, 1]"
    )
    parser.add_argument(
        "--rnn_hidden_dim", type=int, default=100, help="rnn hidden dim"
    )
    parser.add_argument(
        "--model_type", type=str, default="FSARNN", help="baseline MarryUp or FSARNN"
    )
    parser.add_argument(
        "--optimizer", type=str, default="ADAM", help="optimizer SGD or ADAM"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--rnn", type=str, default="RNN", help="rnn type only for MarryUp Baseline"
    )
    parser.add_argument(
        "--re_tag_dim",
        type=int,
        default=20,
        help="re tag dim only for MarryUp Baseline",
    )
    parser.add_argument(
        "--marryup_type",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="training portion, in 0.01, 0.1, 1.0 ",
    )
    parser.add_argument(
        "--random", type=int, default=0, help="if use random initialzation"
    )

    parser.add_argument("--l1", type=float, default=0, help="kd alpha")
    parser.add_argument("--l2", type=float, default=0.0, help="pr constant")

    parser.add_argument(
        "--bidirection", type=int, default=0, help="if the model is bidirection"
    )
    parser.add_argument(
        "--xavier",
        type=int,
        default=1,
        help="if the FSAGRU model initialized using xavier",
    )
    parser.add_argument(
        "--normalize_automata",
        type=str,
    ...
    parser.add_argument(
        ...
        help="CrossEntropy, NormalizeNLL",
    )
    ...
    parser.add_argument(
        "--automata_path_forward", type=str, default="none", help="automata path"
    )
    parser.add_argument(
        "--automata_path_backward", type=str, default="none", help="automata path"
    )

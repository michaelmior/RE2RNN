...
class FSARNNIntegrateEmptyStateSaperateGRU(nn.Module):
    def __init__(
        ...
        if self.farnn == 1:
            self.Wss1 = nn.Parameter(
                (torch.randn((self.S + self.AS, self.S + self.AS))).float(),
                requires_grad=True,
            )
            self.Wrs1 = nn.Parameter(
                (torch.randn((self.R, self.S + self.AS))).float(), requires_grad=True
            )
            self.bs1 = nn.Parameter(
                (torch.ones((1, self.S + self.AS)).float() * self.bias_init),
                requires_grad=True,
            )

            self.Wss2 = nn.Parameter(
                (torch.randn((self.S + self.AS, self.S + self.AS))).float(),
                requires_grad=True,
            )
            self.Wrs2 = nn.Parameter(
                (torch.randn((self.R, self.S + self.AS))).float(), requires_grad=True
            )
            self.bs2 = nn.Parameter(
                (torch.ones((1, self.S + self.AS)).float() * self.bias_init),
                requires_grad=True,
            )
            if self.xavier:
        ...
        self.beta_vec = nn.Parameter(
            torch.tensor([self.beta] * self.R).float(),
            requires_grad=bool(config.train_beta),
        )
        ...
        temp_wildcard = (
            np.random.randn(self.S + self.AS, self.S + self.AS) * config.random_noise
        )  # very small noise
        ...
        self.embed_r = nn.Embedding.from_pretrained(
            torch.from_numpy(embed_r).float(), freeze=(not bool(config.train_V_embed))
        )  # V x R
        self.trans_r_1 = nn.Parameter(
            torch.cat(
                (
                    torch.from_numpy(trans_r_1).float(),
                    torch.randn((self.AS, self.R)) * config.random_noise,
                ),
                dim=0,
            ),
        ...
        self.trans_r_2 = nn.Parameter(
            torch.cat(
                (
                    torch.from_numpy(trans_r_2).float(),
                    torch.randn((self.AS, self.R)) * config.random_noise,
                ),
                dim=0,
            ),
        ...
        self.embed_r_generalized = nn.Parameter(
            torch.matmul(_pinv, _V), requires_grad=bool(config.train_fsa)
        )  # D x R
    ...
    def cal_hidden_bar(self, hidden, L):
        ...
        if self.farnn == 1:
            self.zt = self.gate_activation(
                torch.matmul(hidden, self.Wss1) + torch.matmul(L, self.Wrs1) + self.bs1
            )  # B x S
            rt = self.gate_activation(
                torch.matmul(hidden, self.Wss2) + torch.matmul(L, self.Wrs2) + self.bs2
            )  # B x S
            hidden_bar = torch.einsum(
                "bs,bs->bs", (1 - rt), self.batch_h1
            ) + torch.einsum("bs,bs->bs", rt, hidden)  # reset to previous state
        return hidden_bar
    ...
    def forward(self, input, lengths):
        ...
        all_hidden = (
            torch.zeros((B, L, self.S + self.AS)).cuda()
            if self.is_cuda
            else torch.zeros((B, L, self.S + self.AS))
        )

        for i in range(L):
            emb = all_emb[:, i, :]  # B x D

            L_generalized = torch.matmul(
                emb, self.embed_r_generalized
            )  # B x D, D x R -> B x R
            ...
            hidden_language = torch.matmul(
                LR, self.trans_r_2.transpose(0, 1)
            )  # B x R, R x S  -> B x S
            hidden_wildcard = torch.matmul(
                hidden_bar, self.trans_wildcard
            )  # B x S, S x S -> B x S
            ...
            if self.farnn == 1:
                hidden = torch.einsum(
                    "bs,bs->bs", (1 - self.zt), hidden
                ) + torch.einsum("bs,bs->bs", self.zt, hidden_)
    ...
    def viterbi(self, input, lengths):
        ...
        for i in range(L):
            ...
            Tr1 = torch.einsum(
                "br,rs->brs", L, self.trans_r_1.transpose(1, 0)
            )  # B x R, R x S -> B x R x S
            Tr = torch.einsum(
                "sr,brj->bjs", self.trans_r_2, Tr1
            )  # S x R, B x R x S -> B x S x S
...
class IntentIntegrateSaperateBidirection_B(nn.Module):
    def __init__(
        ...
        self.fsa_rnn_forward = FSARNNIntegrateEmptyStateSaperateGRU(
        ...
        self.fsa_rnn_backward = FSARNNIntegrateEmptyStateSaperateGRU(
        ...
        if not config.random:
            ...
            forward_mat = torch.cat(
                [
                    forward_mat,
                    torch.randn(config.additional_state, forward_mat.size()[1])
                    * config.random_noise,
                ]
            )
            ...
            backward_mat = torch.cat(
                [
                    backward_mat,
                    torch.randn(config.additional_state, backward_mat.size()[1])
                    * config.random_noise,
                ]
            )

            self.mat_forward = nn.Parameter(
                forward_mat, requires_grad=bool(config.train_linear)
            )
            self.bias_forward = nn.Parameter(
                torch.from_numpy(forward_params["bias"]).float(),
                requires_grad=bool(config.train_linear),
            )
            self.mat_backward = nn.Parameter(
                backward_mat, requires_grad=bool(config.train_linear)
            )
            self.bias_backward = nn.Parameter(
                torch.from_numpy(backward_params["bias"]).float(),
                requires_grad=bool(config.train_linear),
            )
        else:
            self.mat_forward = nn.Parameter(
                torch.randn(
                    (
                        forward_params["mat"].shape[0] + config.additional_state,
                        forward_params["mat"].shape[1],
                    )
                ).float(),
                requires_grad=bool(config.train_linear),
            )
            self.bias_forward = nn.Parameter(
                torch.randn(forward_params["bias"].shape).float(),
                requires_grad=bool(config.train_linear),
            )
            self.mat_backward = nn.Parameter(
                torch.randn(
                    (
                        backward_params["mat"].shape[0] + config.additional_state,
                        backward_params["mat"].shape[1],
                    )
                ).float(),
                requires_grad=bool(config.train_linear),
            )
            self.bias_backward = nn.Parameter(
                torch.randn(backward_params["bias"].shape).float(),
                requires_grad=bool(config.train_linear),
            )
    ...
    def forward(self, input_forward, input_backward, lengths):
        ...
        last_hidden_backward = out_backward[
            torch.arange(B), lengths - 1, :
        ]  # select all last hidden
        last_hidden_forward = out_forward[
            torch.arange(B), lengths - 1, :
        ]  # select all last hidden
        ...
        scores_backward = (
            torch.matmul(last_hidden_backward, self.mat_backward) + self.bias_backward
        )
        scores_forward = (
            torch.matmul(last_hidden_forward, self.mat_forward) + self.bias_forward
        )
...
class IntentIntegrateSaperate_B(nn.Module):
    def __init__(
        ...
        self.fsa_rnn = FSARNNIntegrateEmptyStateSaperateGRU(
        ...
        if not config.random:
            self.mat = nn.Parameter(
                torch.cat(
                    [
                        ...
                        torch.randn(config.additional_state, mat.shape[1])
                        * config.random_noise,
                    ]
                ),
                requires_grad=bool(config.train_linear),
            ...
            self.mat = nn.Parameter(
                torch.randn(
                    mat.shape[0] + config.additional_state, mat.shape[1]
                ).float(),
                requires_grad=bool(config.train_linear),
            )
            self.bias = nn.Parameter(
                torch.randn(bias.shape).float(), requires_grad=bool(config.train_linear)
            )
